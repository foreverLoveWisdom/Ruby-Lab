...
# Lessons:
# What if the base classes define the required methods that my future subclasses will never implement?
# What if the subclasses need to implement methods that have nothing to do with the base classes?
# (is this the common problem with inheritance?)

# How to implement:
# 1. Divide the algorithms into small steps first
# 2. Move the common steps into the base classes
# 3. High classes only implement high level steps
# 4. Provide hook (empty or with code) for subclasses to override(optional)
# 5. Implement Subclasses with required methods(specific and different for each subclass.
# (This is why we use this pattern right?)
#
# Pro:
# Easy
# Flexible
...
# Inheritance problems(enforces a particular design, subclasses do not always override the required method)
